...
string containtment(Point p,Rectangle r)
{
    ...
    if (p.x < r.xmin or p.x > r.xmax or p.y < r.ymin or p.y > r.ymax) {
        
        return "outside";
        
    }
    else if (p.x > r.xmin and p.x < r.xmax and p.y > r.ymin and p.y < r.ymax) {
...
int main()
{
    ...
    for (int i = 0; i < vegades; ++i) {
        ...
        Point a = read_point();
        Rectangle b = read_rectangle();
        
        string posicio = containtment(a, b);
